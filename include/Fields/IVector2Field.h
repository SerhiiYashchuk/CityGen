...
public:
  ...
  virtual Vector2 sample(Vector2 pos) const = 0;
};
...
{
  return v == Vector2{} || Utils::dotProduct(prev, v) ? v : Vector2{ -v.x, -v.y };
}
...
{
  const Vector2 k1 = correctedSample(field, pos, prev);
  const Vector2 k2 = correctedSample(field, pos + k1 / 2, prev);
  const Vector2 k3 = correctedSample(field, pos + k2 / 2, prev);
  const Vector2 k4 = correctedSample(field, pos + k3, prev);
